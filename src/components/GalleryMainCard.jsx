...
const GalleryCard = ({subcard, currentSet}) => {

  const [currentIndex, setcurrentIndex] = useState(0)

  const prevImage = () => {
      const isFirstImage = currentIndex === 0;
      const newIndex = isFirstImage ? GalleryData[currentSet].images.length - 1 : currentIndex -1;
      setcurrentIndex(newIndex);
    }
      
  const nextImage = () => {
    const isLastImage = currentIndex === GalleryData[currentSet].images.length - 1;
